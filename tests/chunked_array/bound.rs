...
fn check_mass(bound_chunked: &IndexedIdentifierChunked, mass: f64) {
...
fn saturated_series() {
    ...
    // let series = Series::new(PlSmallStr::from_static("Saturated"), C18U0)
    //     .cast(&BOUND_DATA_TYPE)
    //     .unwrap()
    //     .into_series();
    let bound_chunked = IndexedIdentifierChunked::try_from(&C18);
    assert_eq!(bound_chunked.sized(), 17);
    assert!(bound_chunked.is_saturated());
    assert!(!bound_chunked.is_unsaturated(None));
    // let doubles: Vec<_> = bound_chunked.iter_doubles().collect();
    // assert!(doubles.is_empty());
    // let triples: Vec<_> = bound_chunked.iter_triples().collect();
    // assert!(triples.is_empty());
    let unsaturated: Vec<_> = bound_chunked.iter().unsaturated().collect();
...
//     let series = Series::new(PlSmallStr::from_static("Unsaturated"), C18U3DC9DC12DC15)
//         .cast(&BOUND_DATA_TYPE)
//         .unwrap()
//         .into_series();
//     let bound_chunked = BoundChunked::new(&series);
...
//     let triples: Vec<_> = bound_chunked.iter_triples().collect();
//     assert!(triples.is_empty());
//     let unsaturated: Vec<_> = bound_chunked.iter_unsaturated().collect();
...
//     let series = Series::new(PlSmallStr::from_static("Bounds"), ALL)
//         .cast(&BOUND_DATA_TYPE)
//         .unwrap()
//         .into_series();
