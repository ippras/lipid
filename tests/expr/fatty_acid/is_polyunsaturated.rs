...
macro_rules! check {
    ($identifier:ident, $expected:expr) => {{
        let data_frame = fatty_acid($identifier.clone())?
            ...
            .collect()?;
        let is_polyunsaturated = data_frame["IsPolyunsaturated"].bool()?.get(0).unwrap();
        assert!(is_polyunsaturated == $expected);
    }};
}
