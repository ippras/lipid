...
impl BoundChunked {
    ...
    pub fn filter(&self, predicate: impl Fn(Option<Bound>) -> bool) -> impl Iterator<Item = usize> {
        self.into_iter()
            .enumerate()
            .filter_map(move |(index, bound)| predicate(bound).then_some(index))
    }

    /// Returns an iterator over the indices of double bonds in the bound
    /// chunked array.
    pub fn doubles(&self) -> impl Iterator<Item = usize> {
        self.filter(|bound| bound.is_some_and(Bound::is_double))
    }

    /// Returns an iterator over the indices of triple bonds in the bound
    /// chunked array.
    pub fn triples(&self) -> impl Iterator<Item = usize> {
    ...
    /// chunked array.
    pub fn unsaturated(&self) -> impl Iterator<Item = usize> {
