...
pub trait Permutation {
    ...
    // fn non_stereospecific1<T>(self, f: impl Fn(Series) -> ChunkedArray<T>) -> Expr
    // where
    //     T: PolarsDataType,
    //     for<'a> <T as PolarsDataType>::Physical<'a>: PartialOrd;

    fn non_stereospecific_old(self, f: impl Fn(Expr) -> Expr) -> PolarsResult<Expr>;
}
...
impl Permutation for TriacylglycerolExpr {
    ...
    //     self.0.map(
    //         |column| {
    //             let triacylglycerol = column.struct_()?;
    //             let sn1 = f(triacylglycerol.field_by_name("StereospecificNumber1")?);
    //             let sn2 = f(triacylglycerol.field_by_name("StereospecificNumber2")?);
    //             let sn3 = f(triacylglycerol.field_by_name("StereospecificNumber3")?);
    //             let ((sn1, sn2), sn3): ((Vec<_>, Vec<_>), Vec<_>) = sn1
    //                 .iter()
    //                 .zip(sn2.iter())
    //                 .zip(sn3.iter())
    //                 .map(|((mut sn1, mut sn2), mut sn3)| {
    //                     if sn1 > sn3 {
    //                         swap(&mut sn1, &mut sn3);
    //                     }
    //                     if sn1 > sn2 {
    //                         swap(&mut sn1, &mut sn2);
    //                     }
    //                     if sn3 < sn2 {
    //                         swap(&mut sn3, &mut sn2);
    //                     }
    //                     ((sn1, sn2), sn3)
    //                 })
    //                 .unzip();
    ...
    //                     column.len(),
    //                     [
    //                         Series::new(PlSmallStr::EMPTY, sn1),
    //                         // Series::from_any_values(PlSmallStr::EMPTY, &sn2, true)?,
    //                         // Series::from_any_values(PlSmallStr::EMPTY, &sn3, true)?,
    //                     ]
    //                     .iter(),
    //                 )?
    //                 .into_column(),
    ...
    // fn non_stereospecific0(self, f: impl Fn(Expr) -> Expr) -> PolarsResult<Expr> {
    //     let mut sn1 = self.clone().stereospecific_number1();
    //     let mut sn2 = self.clone().stereospecific_number2();
    //     let mut sn3 = self.clone().stereospecific_number3();
    //     let predicate = f(sn1.clone()).lt_eq(f(sn3.clone()));
    //     // if sn1 > sn3 {
    //     //     swap(&mut sn1, &mut sn3);
    //     // }
    ...
    //     // }
    //     sn1 = ternary_expr(
    //         f(sn1.clone()).gt(f(sn3.clone())),
    //         as_struct(vec![sn1.clone(), sn2.clone(), sn3.clone()]),
    //         as_struct(vec![sn1.clone(), sn2.clone(), sn3.clone()]),
    //     );
    //     // let (sn1, sn3) = (
    //     //     ternary_expr(predicate, sn3, sn1),
...
pub fn test_positional(self, f: impl Fn(Expr) -> Expr) -> Expr {
    ternary_expr(
        f(self.clone().stereospecific_number1()).lt_eq(f(self.clone().stereospecific_number3())),
        as_struct(vec![
            self.clone().stereospecific_number1(),
            self.clone().stereospecific_number2(),
