...
use crate::error::{
    Error::{
        IndicesLengthGreaterThanRange, ParseCarbonKeyword, ParseCarbonValue, StartGreaterThanStop,
    },
    error,
};
...
impl Parse for Carbons {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        if !string.starts_with('C') {
            return error!(ParseCarbonKeyword: input);
        }
        let ident = string.pre(delimiter).parse()?;
        let Ok(carbon) = string.trim_start_matches('C').parse::<u8>() else {
            return error!(ParseCarbonValue: input);
        };
...
mod tests {
    ...
    fn test_parse_full_input() {
        ...
        assert_eq!(input.indices.0.len(), 3);

        let first_index = &input.indices.0[0];
